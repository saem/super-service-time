...
internal fun runConsole(consoleApp: Console, args: Array<String>): Int {
    try {
        consoleApp.run(args)
...
internal class OutWriter(val writerHistory: MutableList<ProcessAction>) : Writer() {
    override fun flush() {
...
internal fun PretendExitProcessWithHistory(processHistory: MutableList<ProcessAction>): (Int) -> Nothing =
...
internal fun historyIsValid(history: List<ProcessAction>): Boolean {
...
internal fun writtenHistoryToString(history: List<ProcessAction>): String {
...
internal sealed class ProcessAction

internal data class ExitProcess(val exitCode: Int): ProcessAction()

internal sealed class WriterMessage: ProcessAction()
internal data class Flush(val dummy: Int = 0) : WriterMessage()
internal data class Close(val dummy: Int = 1) : WriterMessage()
internal data class Write(val chars: CharArray?, private val start: Int, val finish: Int) : WriterMessage() {
